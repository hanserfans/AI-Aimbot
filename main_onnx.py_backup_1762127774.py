...
def main():
    ...
    while win32api.GetAsyncKeyState(ord(aaQuitKey)) == 0:
        ...
        if len(targets) > 0 and head_x_320 is not None and head_y_320 is not None:
            ...
            print(f"[FRAME_SYNC] 使用同帧数据 - 头部位置: ({head_x_for_aiming}, {head_y_for_aiming}), 准星位置: ({crosshair_x_for_aiming}, {crosshair_y_for_aiming})")
        ...
        # Live Feed显示
        if showLiveFeed and 'original_img' in locals():
            # 🔥 移除帧率限制，确保头部标记立即跟随帧变化，无延迟显示
            # 🔥 使用正在进行头部位置判断的原始帧（确保先判断再显示）
            print(f"[FRAME_SYNC] Live Feed使用正在进行头部位置判断的原始帧（无帧率限制）")
            live_feed_img = original_img  # 使用正在进行头部位置计算的原始帧
