...
# 实时头部位置跟踪系统导入
from realtime_head_position_tracker import RealtimeHeadPositionTracker, create_realtime_head_position_tracker
...
# from high_performance_screenshot_system import HighPerformanceScreenshotSystem
    ...
    print("[INFO] 📷 使用简单MSS截图系统")
except ImportError as e:
    print(f"[WARNING] 高性能截图系统加载失败: {e}")
    HIGH_PERFORMANCE_SCREENSHOT_AVAILABLE = False

# 导入多线程AI处理系统
# 禁用复杂的多线程AI处理系统，使用简单处理
MULTI_THREADED_AI_AVAILABLE = False
print("[INFO] 📷 使用简单AI处理系统，避免多线程复杂性")
...
# 独立开火检测系统
try:
    from independent_fire_detection_system import IndependentFireDetectionSystem
...
# 实时头部位置显示系统
try:
    from realtime_head_display_fix import RealtimeHeadDisplaySystem
    REALTIME_HEAD_DISPLAY_AVAILABLE = True
    print("[INFO] ✅ 实时头部位置显示系统已加载到主程序")
except ImportError as e:
    print(f"[WARNING] 实时头部位置显示系统加载失败: {e}")
    REALTIME_HEAD_DISPLAY_AVAILABLE = False

# 智能自适应移动系统初始化
ADAPTIVE_MOVEMENT_AVAILABLE = True  # 先定义变量
if ADAPTIVE_MOVEMENT_AVAILABLE:
    try:
        ...
        # 注意：这里暂时使用None，因为move_mouse_direct函数还未定义
        # 实际初始化将在main()函数中完成
        print("[INFO] ✅ 智能自适应移动系统已加载到主程序")
    except Exception as e:
        print(f"[WARNING] 智能自适应移动系统加载失败: {e}")
        ADAPTIVE_MOVEMENT_AVAILABLE = False
else:
    print("[WARNING] 智能自适应移动系统不可用")
...
# 🎯 新增：直接一步移动系统（专为Arduino Leonardo优化）
...
# 🧠 新增：智能移动状态管理系统（解决移动期间检测丢失问题）
from intelligent_movement_manager import create_intelligent_movement_manager
intelligent_movement_manager = create_intelligent_movement_manager(arduino_limit=127)

def move_mouse(x, y, use_smooth=True, use_non_blocking=True, use_adaptive=True, use_direct_single_step=False):
    """
    统一的鼠标移动函数，支持多种移动模式
    ...
    Args:
        ...
        use_direct_single_step: 是否使用直接一步移动（默认False，最高优先级）
    """
    global adaptive_movement_system
    
    # 🎯 最高优先级：直接一步移动（专为Arduino Leonardo优化）
    if use_direct_single_step:
...
def calculate_smoothed_head_position(target_x, target_y, box_height):
    """计算头部位置（已禁用平滑）- 320坐标系版本"""
    # 🔥 修复头部偏移计算：头部应该在目标框的上方约0.38处
    # 320坐标系下使用标准偏移量0.38
    headshot_offset = box_height * 0.38  # 320坐标系下的标准偏移量
    head_x = target_x
    head_y = target_y - headshot_offset  # 向上偏移到头部位置
    
    # 调试信息：打印头部位置计算过程（320坐标系）
    print(f"[HEAD_CALC_320_DEBUG] 目标中心({target_x:.1f}, {target_y:.1f}), 框高度{box_height:.1f}, 偏移量{headshot_offset:.1f}, 头部位置({head_x:.1f}, {head_y:.1f})")
    
    # 🚫 已禁用头部位置平滑系统 - 避免不同目标间的错误平滑
    # 直接返回原始计算的头部位置，确保每次检测都是独立的
    print(f"[HEAD_NO_SMOOTH_DEBUG] 使用原始头部位置({head_x:.1f}, {head_y:.1f}) - 平滑已禁用")
...
def main():
    ...
    # 初始化独立开火检测系统
    global fire_detection_integration
    fire_detection_integration = None
    if INDEPENDENT_FIRE_DETECTION_AVAILABLE:
        try:
            fire_detection_integration = FireDetectionIntegration(
                main_auto_fire_func=auto_fire,
                main_auto_fire_fast_func=auto_fire_fast
            )
            print("[FIRE_DETECTION] ✅ 独立开火检测系统已初始化")
            print("[FIRE_DETECTION] 🎯 开火检测与移动系统已解耦")
            print("[FIRE_DETECTION] 🔄 实时检测最新帧中的头部位置")
        except Exception as e:
    ...
    # 初始化实时头部位置显示系统
    global realtime_head_display
    realtime_head_display = None
    if REALTIME_HEAD_DISPLAY_AVAILABLE:
        try:
            realtime_head_display = RealtimeHeadDisplaySystem()
            print("[HEAD_DISPLAY] ✅ 实时头部位置显示系统已初始化")
            print("[HEAD_DISPLAY] 🎯 Live Feed头部位置与检测同步")
            print("[HEAD_DISPLAY] 🔒 移动状态锁定机制已启用")
            print("[HEAD_DISPLAY] 🔄 实时帧同步显示已启用")
        except Exception as e:
            print(f"[HEAD_DISPLAY] ❌ 实时头部位置显示系统初始化失败: {e}")
            realtime_head_display = None
    else:
        print("[HEAD_DISPLAY] ⚠️ 实时头部位置显示系统不可用")
    
    # 初始化实时头部位置跟踪系统
    global realtime_head_tracker
    try:
        realtime_head_tracker = create_realtime_head_position_tracker(
            movement_lock_duration=0.3,  # 移动锁定持续时间300ms
            position_history_size=50,    # 位置历史记录大小
            visualization_enabled=True   # 启用可视化
        )
        print("[HEAD_TRACKER] ✅ 实时头部位置跟踪系统已初始化")
        print("[HEAD_TRACKER] 🔒 移动锁定机制已启用 (300ms)")
        print("[HEAD_TRACKER] 📊 位置历史记录已启用 (50帧)")
        print("[HEAD_TRACKER] 🎨 可视化功能已启用")
    except Exception as e:
        print(f"[HEAD_TRACKER] ❌ 实时头部位置跟踪系统初始化失败: {e}")
        realtime_head_tracker = None
    ...
    if ENHANCED_TARGET_SELECTION_AVAILABLE:
        try:
            enhanced_target_system = create_enhanced_target_system()
            print("[INFO] ✅ 增强目标选择系统已初始化")
            print(f"[INFO] 距离权重因子: {enhanced_target_system.distance_weight_factor}")
            print(f"[INFO] 移动锁定时长: {enhanced_target_system.movement_lock_duration}s")
            print(f"[INFO] 移动完成阈值: {enhanced_target_system.movement_completion_threshold}px")
        except Exception as e:
            print(f"[WARNING] 增强目标选择系统初始化失败: {e}")
            enhanced_target_system = None
    ...
    if DETECTION_STABILITY_AVAILABLE:
        ...
        stability_system = create_stability_system(
            ...
            min_detection_count=1,      # 降低到1次检测即可稳定，适应帧丢弃机制
    ...
    # 使用简单MSS截图系统，无需复杂初始化
    print("[INFO] 📷 简单MSS截图系统已准备就绪")
    print(f"   • 截图区域: {region}")
    print(f"   • 截图方式: 直接MSS捕获")
    
    # 初始化多线程AI处理系统（增强版，集成帧时间顺序管理）
    multi_ai_processor = None
    # 使用简单AI处理，不启用复杂的多线程系统
    print("[INFO] 📷 使用简单AI处理系统，避免多线程卡顿")
    ...
    def find_best_target_with_lock(targets, current_time):
        ...
        if locked_target:
            # 安全检查：确保 locked_target 包含必要的键
            if 'head_x' not in locked_target or 'head_y' not in locked_target:
                print(f"[WARNING] locked_target 缺少 head_x 或 head_y 键，重置锁定目标")
                locked_target = None
                lock_start_time = 0
            else:
    ...
    def fire_check_callback():
            """移动过程中的开火检测回调函数"""
            try:
                # 获取当前准星位置（320坐标系）- 统一坐标系统修复
                crosshair_x_320 = cWidth   # 160 (320/2)
                crosshair_y_320 = cHeight  # 160 (320/2)
    ...
    while win32api.GetAsyncKeyState(ord(aaQuitKey)) == 0:
        ...
        if win32api.GetAsyncKeyState(ord('R')) & 0x8000:
            ...
            if current_time - last_report_time > 2:  # 防止重复触发
                ...
                # 显示独立开火检测系统统计信息
                if INDEPENDENT_FIRE_DETECTION_AVAILABLE and fire_detection_integration:
                    try:
                        stats = fire_detection_integration.get_statistics()
                        print(f"[FIRE_DETECTION] 🔥 独立开火检测系统统计:")
                        print(f"[FIRE_DETECTION] 📊 检测次数: {stats.get('detection_count', 0)}")
                        print(f"[FIRE_DETECTION] 🎯 开火次数: {stats.get('fire_count', 0)}")
                        print(f"[FIRE_DETECTION] ⏱️ 平均检测时间: {stats.get('avg_detection_time', 0):.3f}ms")
                        print(f"[FIRE_DETECTION] 🎮 系统状态: {'运行中' if stats.get('is_running', False) else '已停止'}")
                    except Exception as e:
                        print(f"[FIRE_DETECTION] ❌ 获取统计信息失败: {e}")
        ...
        if npImg is None:
            print("[WARNING] 截图失败，创建占位图像以保持Live Feed显示")
            # 创建一个占位图像，确保Live Feed能够显示错误状态
            npImg = np.zeros((640, 640, 3), dtype=np.uint8)
            # 在占位图像上显示错误信息
            cv2.putText(npImg, "SCREENSHOT FAILED", (150, 300), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 3)
            cv2.putText(npImg, "Retrying...", (250, 350), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
        ...
        # 🔥 保存真正的原始图像用于Live Feed显示（在任何处理之前）
        raw_original_img = npImg.copy()
        print(f"[LIVE_FEED_FIX] 保存原始图像用于Live Feed: {raw_original_img.shape[1]}x{raw_original_img.shape[0]}")
        ...
        # Store original image for coordinate calculations (after mask)
        ...
        # GPU加速后处理（优先使用统一内存处理器）
            ...
            try:
                ...
                if gpu_processor is not None:
                    try:
                        ...
                        print(f"[WARNING] 传统GPU后处理也失败，回退到CPU: {e2}")
                        # 回退到CPU后处理
                        nms_result = non_max_suppression(torch.from_numpy(outputs[0]), confidence, 0.45, None, False, max_det=1000)[0]
                        if isinstance(nms_result, torch.Tensor):
                            nms_result = nms_result.cpu().numpy()
                        targets = pd.DataFrame(nms_result)
                        if len(targets) == 0:
                            targets = pd.DataFrame(columns=['current_mid_x', 'current_mid_y', 'width', "height", "confidence"])
                        else:
            ...
            # 添加置信度过滤，过滤掉低置信度目标
            if len(targets) > 0:
                # 设置最小置信度阈值（比NMS阈值高一些，确保质量）
                MIN_CONFIDENCE_THRESHOLD = 0.3  # 最小置信度阈值
                original_count = len(targets)
                targets = targets[targets['confidence'] >= MIN_CONFIDENCE_THRESHOLD]
                filtered_count = len(targets)
                
                if original_count != filtered_count:
                    print(f"[CONFIDENCE_FILTER] 原始目标: {original_count} -> 过滤后: {filtered_count} (阈值: {MIN_CONFIDENCE_THRESHOLD})")
                    if filtered_count > 0:
                        print(f"[CONFIDENCE_FILTER] 保留置信度范围: {targets['confidence'].min():.3f} - {targets['confidence'].max():.3f}")
        ...
        # 检测图像的中心坐标（用于距离计算）- 现在使用640坐标系
        detection_center = [original_capture_size / 2, original_capture_size / 2]  # [320, 320] for 640x640 detection box
        ...
        if len(targets) > 0:
            ...
            # 将320坐标系的检测结果转换为640坐标系
            # 缩放因子：640 / 320 = 2.0
            scale_factor = original_capture_size / npImg.shape[0]  # 640 / 320 = 2.0
            print(f"[COORD_FIX] 坐标缩放因子: {scale_factor} (从{npImg.shape[0]}到{original_capture_size})")
            
            targets['current_mid_x'] = targets['current_mid_x'] * scale_factor
            targets['current_mid_y'] = targets['current_mid_y'] * scale_factor
            targets['width'] = targets['width'] * scale_factor
            targets['height'] = targets['height'] * scale_factor
            
            # 计算距离检测图像中心的距离（用于排序）
            # 确保数据类型兼容性 - 使用 float16
            targets['current_mid_x'] = targets['current_mid_x'].astype(np.float16)
            targets['current_mid_y'] = targets['current_mid_y'].astype(np.float16)
            targets['confidence'] = targets['confidence'].astype(np.float16)
            
            # 🎯 简化目标选择：使用欧几里得距离选择最近目标
            if (centerOfScreen):
                # 计算到准星中心的距离（使用头部位置）
                center_screen = [cWidth, cHeight]  # 准星位置
                
                # 计算头部位置并计算到准星的距离
                targets["dist_from_center"] = np.sqrt(
                    (targets.current_mid_x - center_screen[0])**2 + 
                    (targets.current_mid_y - targets.height * 0.38 - center_screen[1])**2
                )
                
                # 按距离排序（距离越近越好）
                targets = targets.sort_values("dist_from_center")
                print(f"[TARGET_SELECT] 🎯 检测到 {len(targets)} 个目标，选择离准星最近的目标")
                print(f"[TARGET_SELECT] 最近目标距离: {targets.iloc[0]['dist_from_center']:.1f} 像素")
            else:
                # 如果不是中心模式，按置信度排序
                targets = targets.sort_values("confidence", ascending=False)
                print(f"[TARGET_SELECT] 🎯 检测到 {len(targets)} 个目标，选择置信度最高的目标")
                print(f"[TARGET_SELECT] 最高置信度: {targets.iloc[0]['confidence']:.3f}")
            ...
            def calculate_distance_to_crosshair(target_x, target_y, box_height, crosshair_x, crosshair_y):
                ...
                Returns:
                    float: 头部到准星的距离
                """
                # 确保所有输入都是数值类型，避免Tensor类型错误
                target_x = float(target_x) if hasattr(target_x, 'item') else float(target_x)
                target_y = float(target_y) if hasattr(target_y, 'item') else float(target_y)
                box_height = float(box_height) if hasattr(box_height, 'item') else float(box_height)
                ...
                return float(distance)  # 确保返回float类型
        ...
        # 🔥 更新独立开火检测系统的帧数据（在处理任何目标之前）
        if INDEPENDENT_FIRE_DETECTION_AVAILABLE and fire_detection_integration:
            try:
                fire_detection_integration.update_frame_data(targets, npImg)
                print(f"[FIRE_DETECTION] 🎯 已更新独立开火检测帧数据，目标数量: {len(targets)}")
            except Exception as e:
                print(f"[FIRE_DETECTION] ❌ 更新帧数据失败: {e}")
        ...
        if len(targets) > 0:
            ...
            if movement_locked_target is not None:
                # 坐标转换：640坐标系 -> 320坐标系（movement_locked_target存储的是640坐标系）
                xMid = movement_locked_target['x'] / scale_factor  # 640 -> 320
                yMid = movement_locked_target['y'] / scale_factor  # 640 -> 320
                box_height = movement_locked_target['height'] / scale_factor  # 640 -> 320
                ...
                if selected_target is not None:
                    # 坐标转换：640坐标系 -> 320坐标系（用于后续头部偏移计算）
                    xMid = selected_target['current_mid_x'] / scale_factor  # 640 -> 320
                    yMid = selected_target['current_mid_y'] / scale_factor  # 640 -> 320
                    box_height = selected_target['height'] / scale_factor   # 640 -> 320
                    ...
                    if (centerOfScreen):
                        ...
                        targets = targets.sort_values("confidence", ascending=False)
                        print(f"[TARGET_SELECT] 备选逻辑：检测到 {len(targets)} 个目标，选择置信度最高的目标")
                    
                    # 坐标转换：640坐标系 -> 320坐标系（用于后续头部偏移计算）
                    xMid = targets.iloc[0].current_mid_x / scale_factor  # 640 -> 320
                    yMid = targets.iloc[0].current_mid_y / scale_factor  # 640 -> 320
                    box_height = targets.iloc[0].height / scale_factor   # 640 -> 320
            ...
            if target_lock_enabled and locked_target and selected_target is not None:
                ...
                if detection_lost and target_lock_enabled:
                    # 🧠 智能检测丢失处理：移动期间忽略短时间检测丢失
                    if intelligent_movement_manager.should_ignore_detection_loss():
                        # 使用锁定的目标继续移动
                        locked_target = intelligent_movement_manager.get_locked_target()
                        if locked_target:
                            print(f"[INTELLIGENT_MOVEMENT] 🛡️ 移动期间检测丢失，使用锁定目标继续")
                            head_x_320 = locked_target['x']
                            head_y_320 = locked_target['y']
                            head_source = "LOCKED_TARGET_DURING_MOVEMENT"
                        else:
                            print(f"[PURE_HEAD_MAIN] 检测丢失，纯净系统跳过瞄准处理（无历史记忆补偿）")
                            head_x_320 = None  # 标记为无有效头部位置
                            head_y_320 = None
                            head_source = "DETECTION_LOST"
                    else:
                        # 纯净系统：检测丢失时不使用历史记忆或预测
                        # 跳过瞄准处理，但保留Live Feed显示
                        print(f"[PURE_HEAD_MAIN] 检测丢失，纯净系统跳过瞄准处理（无历史记忆补偿）")
                        head_x_320 = None  # 标记为无有效头部位置
                        head_y_320 = None
                        head_source = "DETECTION_LOST"
                    # 不使用continue，让程序继续执行Live Feed显示
                else:
                    # 检测正常时：使用实际检测到的位置
            ...
            # 🔧 坐标系统统一修复：确保瞄准计算和LIVE框显示使用相同的坐标基准
            if head_x_320 is not None and head_y_320 is not None:
                # 准星位置在320坐标系下的中心点
                crosshair_x_320 = cWidth  # 160 (320/2)
                crosshair_y_320 = cHeight  # 160 (320/2)
                
                # 🎯 修复准星位置偏移：准星位置也应该考虑头部偏移
                # 计算头部偏移量（与头部位置计算保持一致）
                headshot_offset_320 = box_height * (0.38 if headshot_mode else 0.2)
                crosshair_y_320_adjusted = crosshair_y_320 - headshot_offset_320
                
                # 计算鼠标移动量（320坐标系）- 使用调整后的准星位置
                mouseMove = [head_x_320 - crosshair_x_320, head_y_320 - crosshair_y_320_adjusted]
                
                print(f"[COORDINATE_UNIFIED] 320坐标系 - 头部: ({head_x_320:.1f}, {head_y_320:.1f}), 准星: ({crosshair_x_320:.1f}, {crosshair_y_320:.1f}→{crosshair_y_320_adjusted:.1f}), 移动: ({mouseMove[0]:.1f}, {mouseMove[1]:.1f})")
                
                # 🎯 更新实时头部位置跟踪系统
                # 简化头部位置更新，移除复杂的实时头部跟踪器
                print(f"[HEAD_TRACKER] 头部位置更新: ({head_x_320:.1f}, {head_y_320:.1f}), 接受: True")
            else:
                mouseMove = None
                print("[MOUSE_MOVE] 跳过鼠标移动计算：头部位置为None")
            ...
            if head_x_320 is not None and head_y_320 is not None and mouseMove is not None:
                ...
                print(f"[UNIFIED_AIMING_DEBUG] 跳过瞄准计算：头部位置或鼠标移动为None")
        ...
        if visuals:
            ...
            for i in range(0, len(targets)):
                ...
                # 确保width和height是数值类型，避免Tensor类型错误
                width_val = float(targets.iloc[i]["width"]) if hasattr(targets.iloc[i]["width"], 'item') else float(targets.iloc[i]["width"])
                height_val = float(targets.iloc[i]["height"]) if hasattr(targets.iloc[i]["height"], 'item') else float(targets.iloc[i]["height"])
                halfW = round(width_val / 2)
                halfH = round(height_val / 2)
                
                # 确保坐标是数值类型，避免Tensor类型错误
                midX_val = float(targets.iloc[i]['current_mid_x']) if hasattr(targets.iloc[i]['current_mid_x'], 'item') else float(targets.iloc[i]['current_mid_x'])
                midY_val = float(targets.iloc[i]['current_mid_y']) if hasattr(targets.iloc[i]['current_mid_y'], 'item') else float(targets.iloc[i]['current_mid_y'])
                
                (startX, startY, endX, endY) = int(midX_val - halfW), int(midY_val - halfH), int(midX_val + halfW), int(midY_val + halfH)

                idx = 0
                # draw the bounding box and label on the frame
                label = "{}: {:.2f}%".format(
                    "Human", targets.iloc[i]["confidence"] * 100)
        ...
        if showLiveFeed and display_img is not None:
            ...
            # 🛡️ 安全绘制校准后的准星标记，添加异常处理防止死锁
            try:
                if display_img is not None and display_img.size > 0:
                    # 检查坐标是否在图像范围内
                    img_height, img_width = display_img.shape[:2]
                    if 0 <= crosshair_x_display < img_width and 0 <= crosshair_y_display < img_height:
                        ...
                        print("[CROSSHAIR_DRAW] ✅ 准星标记绘制成功")
                    else:
                        print(f"[CROSSHAIR_DRAW] ⚠️ 准星坐标超出图像范围: ({crosshair_x_display}, {crosshair_y_display}), 图像尺寸: {img_width}x{img_height}")
                else:
                    print("[CROSSHAIR_DRAW] ⚠️ display_img无效，跳过准星绘制")
            except Exception as e:
                print(f"[CROSSHAIR_DRAW_ERROR] 准星绘制异常: {e}")
            ...
            try:
                if display_img is not None and display_img.size > 0:
                    ...
                    print("[CROSSHAIR_INFO] ✅ 准星信息显示成功")
            except Exception as e:
                print(f"[CROSSHAIR_INFO_ERROR] 准星信息显示异常: {e}")
            ...
            try:
                if display_img is not None and display_img.size > 0:
                    ...
                    print("[INFO_DISPLAY] ✅ 系统信息显示成功")
            except Exception as e:
                print(f"[INFO_DISPLAY_ERROR] 系统信息显示异常: {e}")
            
            print("[CROSSHAIR_SECTION] ✅ 准星校准和显示部分完成，继续执行后续代码")
        ...
        # 🎯 右键锁头和扳机检测系统
        if len(targets) > 0 and head_x_320 is not None and head_y_320 is not None:
            # 检测按键状态
            caps_lock_pressed = win32api.GetKeyState(0x14) & 0x0001  # Caps Lock - 纯扳机键
            right_mouse_pressed = win32api.GetKeyState(0x02) & 0x8000  # 鼠标右键 - 瞄准+扳机
            
            # 直接使用简单的头部坐标，避免复杂的实时头部位置系统
            ...
            print(f"[SIMPLE_HEAD] 使用简单头部坐标 - 头部位置: ({head_x_for_aiming}, {head_y_for_aiming}), 准星位置: ({crosshair_x_for_aiming}, {crosshair_y_for_aiming})")
            
            # 鼠标右键：瞄准+扳机模式
            if right_mouse_pressed:
                print(f"[DEBUG] 🖱️ 鼠标右键已按下，激活瞄准系统")
                
                # 修复坐标计算：鼠标移动方向 = 目标位置 - 准星位置
                mouseMove = [head_x_for_aiming - 160, head_y_for_aiming - 160]
                ...
                if movement is not None:
                    ...
                    if trigger_system.enabled and not fire_executed:
                        # 🎯 关键修复：确保头部位置和鼠标检测使用同一帧数据
                        current_frame_head_x = head_x_for_aiming
                        current_frame_head_y = head_y_for_aiming
                        current_frame_crosshair_x = crosshair_x_for_aiming
                        current_frame_crosshair_y = crosshair_y_for_aiming
                        
                        print(f"[FIRE_FIRST] 🔥 移动前开火检测:")
                    ...
                    # 检查是否已经开火
                    if not fire_executed:
                        # 🧠 智能移动管理：计算最优移动策略
                        if abs(move_x) > 0.1 or abs(move_y) > 0.1:  # 避免微小移动
                            # 计算最优移动量（考虑Arduino限制和移动精度）
                            optimal_x, optimal_y, needs_multiple = intelligent_movement_manager.calculate_optimal_movement(
                                move_x, move_y
                            )
                            
                            print(f"🧠 [INTELLIGENT_MOVE] 智能移动: 原始({move_x:.1f}, {move_y:.1f}) -> 优化({optimal_x:.1f}, {optimal_y:.1f})")
                            if needs_multiple:
                                print(f"🧠 [INTELLIGENT_MOVE] 需要多步移动，当前为第一步")
                            
                            # 开始移动状态管理
                            locked_target = {
                                'x': head_x_for_aiming, 
                                'y': head_y_for_aiming,
                                'head_x': head_x_for_aiming,
                                'head_y': head_y_for_aiming
                            }
                            movement_id = intelligent_movement_manager.start_movement(optimal_x, optimal_y, locked_target)
                            
                            # 🎯 使用非阻塞平滑移动（更自然，支持移动中开火回调）
                            if move_mouse(
                                optimal_x,
                                optimal_y,
                                use_smooth=True,
                                ...
                                use_adaptive=False,
                                use_direct_single_step=False
                            ):
                                print("[DEBUG] ✅ 非阻塞平滑移动成功")
                                intelligent_movement_manager.end_movement(movement_id, True)
                                
                                # 🔓 通知实时头部位置显示系统移动结束
                                if realtime_head_display is not None:
                                    realtime_head_display.on_movement_end()
                                
                                # 🎯 通知实时头部位置跟踪系统移动完成
                                if realtime_head_tracker is not None:
                                    realtime_head_tracker.on_movement_complete()
                            else:
                                # 简化移动完成处理
                                print("[DEBUG] ❌ 智能一步移动失败")
                                intelligent_movement_manager.end_movement(movement_id, False)
                        else:
                        ...
                        if trigger_system.enabled and not fire_executed:
                            ...
                            current_frame_head_y = head_y_for_aiming
                            # 计算移动后的准星位置
                            current_frame_crosshair_x = crosshair_x_for_aiming + int(move_x) if 'move_x' in locals() else crosshair_x_for_aiming
                            current_frame_crosshair_y = crosshair_y_for_aiming + int(move_y) if 'move_y' in locals() else crosshair_y_for_aiming
                            
                            print(f"[FIRE_AFTER] 🔥 移动后开火检测:")
            ...
            if len(targets) > 0:
                ...
                for idx in range(len(targets)):
                    ...
                    if target_lock_enabled and locked_target and idx == 0:
                        ...
                        # 直接计算实时头部位置，无历史记忆
                        headshot_offset_320 = box_height_320 * 0.38
                        head_x_320 = target_x_320
                        head_y_320 = target_y_320 - headshot_offset_320
                        print(f"[HEAD_CALC_DISPLAY] 计算头部位置: ({head_x_320:.1f}, {head_y_320:.1f})")

        # Live Feed显示
        # Live Feed窗口显示（使用实时头部位置显示系统）
        current_time = time.time()
        
        if showLiveFeed:
            # 🚀 高性能Live Feed - 减少调试输出和不必要的处理
            if raw_original_img is not None:
                # 直接在原图上绘制，避免完整拷贝
                live_display_img = raw_original_img.copy()
                
                # 计算Live显示坐标系转换比例
                scale_x = live_display_img.shape[1] / 320  # 从320坐标系转换到Live显示坐标系
                scale_y = live_display_img.shape[0] / 320  # 从320坐标系转换到Live显示坐标系
                ...
                cv2.putText(live_display_img, f"Mouse: ({mouse_x}, {mouse_y})", 
                           (10, info_y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 1)
                cv2.putText(live_display_img, f"Crosshair: ({crosshair_x}, {crosshair_y})", 
                           (10, info_y_offset + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 1)
                ...
                # 目标数量显示
                target_count = len(targets) if 'targets' in locals() and targets is not None else 0
                cv2.putText(live_display_img, f"Targets: {target_count}", 
                           (10, info_y_offset + 50), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
                
                # 高效目标绘制 - 完整的坐标转换和头部位置显示
                if 'targets' in locals() and targets is not None and len(targets) > 0:
                    print(f"[LIVE_FEED_DEBUG] 开始绘制 {len(targets)} 个目标")
                    
                    # 遍历所有目标进行完整的坐标转换和绘制（移除数量限制）
                    for idx in range(len(targets)):  # 显示所有目标，不限制数量
                        try:
                            row = targets.iloc[idx]
                            
                            # 获取640坐标系下的目标数据（已经转换过）
                            target_x_640 = float(row['current_mid_x']) if hasattr(row['current_mid_x'], 'item') else float(row['current_mid_x'])
                            target_y_640 = float(row['current_mid_y']) if hasattr(row['current_mid_y'], 'item') else float(row['current_mid_y'])
                            box_height_640 = float(row['height']) if hasattr(row['height'], 'item') else float(row['height'])
                            box_width_640 = float(row['width']) if hasattr(row['width'], 'item') else float(row['width'])
                            target_confidence = float(row['confidence']) if hasattr(row['confidence'], 'item') else float(row['confidence'])
                            
                            # 🔥 关键修复：直接使用640坐标系的坐标，无需额外缩放
                            # 因为检测框坐标已经在2065-2068行被正确转换为640坐标系
                            # 而live_display_img也是640x640尺寸，所以是1:1映射
                            target_x = target_x_640
                            target_y = target_y_640
                            box_height = box_height_640
                            box_width = box_width_640
                            ...
                            cv2.rectangle(live_display_img, (x1, y1), (x2, y2), color, 2)
                            ...
                            # 🔥 关键修复：计算并绘制头部标记（使用320坐标系）
                            # 需要将640坐标系的数据转换回320坐标系进行头部位置计算
                            current_mid_x_320 = target_x_640 / scale_factor
                            current_mid_y_320 = target_y_640 / scale_factor
                            height_320 = box_height_640 / scale_factor
                            head_x_320, head_y_320 = calculate_smoothed_head_position(current_mid_x_320, current_mid_y_320, height_320)
                            
                            # 转换320坐标系的头部位置到640坐标系用于显示
                            head_x_640 = head_x_320 * scale_factor
                            head_y_640 = head_y_320 * scale_factor
                            
                            # 转换320坐标系的头部位置直接到Live显示坐标系
                            ...
                            if target_confidence >= confidence:
                                ...
                                # 显示320坐标系的头部位置数值，但在640坐标系位置显示
                                cv2.putText(live_display_img, f"HEAD({head_x_320:.0f},{head_y_320:.0f})", (int(head_x) + 10, int(head_y)), 
                                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
                                
                                # 只为前3个目标打印详细调试信息，避免日志过多
                                if idx < 3:
                                    print(f"[LIVE_FEED_COORDINATE_FINAL] 目标{idx} 坐标转换:")
                                    print(f"  640坐标系: 目标({target_x_640:.1f}, {target_y_640:.1f}) 头部({head_x_640:.1f}, {head_y_640:.1f})")
                                    print(f"  LIVE框显示坐标系: 目标({target_x:.1f}, {target_y:.1f}) 头部({head_x:.1f}, {head_y:.1f})")
                                    print(f"  缩放比例: {scale_x:.1f}x{scale_y:.1f}")
                                    print(f"  LIVE框尺寸: {live_display_img.shape[1]}x{live_display_img.shape[0]}")
                                    print(f"  置信度: {target_confidence:.3f}")
                            
                        except Exception as e:
                            print(f"[LIVE_FEED_ERROR] 目标{idx}绘制失败: {e}")
                            continue  # 跳过有问题的目标，继续处理下一个
                else:
                    # 无目标提示
                    cv2.putText(live_display_img, "NO TARGETS", 
                               (10, info_y_offset + 75), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 1)
                
                # 简化可视化处理，移除复杂的实时头部跟踪器
                # 直接使用现有的 live_display_img
                ...
                print(f"[LIVE_FEED_DEBUG] original_img不可用，跳过显示")
            
        elif not showLiveFeed:
            # 即使不显示Live Feed，也要安全检查退出键
            try:
                key = cv2.waitKey(1) & 0xFF
                if key == ord('q') or key == 27:  # Q键或ESC键
                    print("[MAIN] 用户请求退出程序...")
                ...
                print(f"[MAIN_ERROR] 键盘检测异常: {e}")
                # 忽略键盘检测错误，继续运行

    # 🛡️ 强制安全退出和资源清理
    print("\n" + "="*60)
    print("🛡️ 开始安全退出程序和资源清理...")
    print("="*60)
    
    # 1. 首先关闭所有OpenCV窗口
    try:
        cv2.destroyAllWindows()
        print("[CLEANUP] ✅ OpenCV窗口已关闭")
    except Exception as e:
        print(f"[CLEANUP] ⚠️ OpenCV窗口关闭失败: {e}")
    
    # 2. 停止所有线程和异步操作
    try:
        import threading
        active_threads = threading.active_count()
        print(f"[CLEANUP] 当前活跃线程数: {active_threads}")
    except:
        pass

    # 清理独立开火检测系统
    if INDEPENDENT_FIRE_DETECTION_AVAILABLE and fire_detection_integration is not None:
        try:
            fire_detection_integration.stop()
            print("[INFO] 🎯 独立开火检测系统已停止")
        except Exception as e:
            print(f"[WARNING] 独立开火检测系统清理失败: {e}")
    ...
    try:
        gc.collect()
        print("[CLEANUP] ✅ 垃圾回收完成")
    except Exception as e:
        print(f"[CLEANUP] ⚠️ 垃圾回收失败: {e}")
        pass
    ...
    try:
        ...
        print("[CLEANUP] ✅ 最终OpenCV清理完成")
    except:
        pass
    
    print("[EXIT] 🎯 AI瞄准系统安全退出完成！")
